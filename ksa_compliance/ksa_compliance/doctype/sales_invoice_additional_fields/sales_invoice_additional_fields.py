...
class SalesInvoiceAdditionalFields(Document):
    ...
    def _get_invoice_type(self, settings: ZATCABusinessSettings) -> InvoiceType:
        # Check if this is a clearance redirect to simplified invoice
        # Use the dedicated clearance_redirect field
        clearance_redirect = getattr(self, 'clearance_redirect', False)
        
        logger.info(f'_get_invoice_type: clearance_redirect field = {clearance_redirect}')
        
        if clearance_redirect:
            logger.info(f'_get_invoice_type: Returning Simplified due to clearance redirect')
            return 'Simplified'
    ...
    def before_insert(self):
        prev_integration_status = self.integration_status
        ...
        logger.info(f'before_insert: clearance_redirect field = {clearance_redirect}')
        logger.info(f'before_insert: previous integration status = {prev_integration_status}, current integration status = {self.integration_status}')
        
        if not clearance_redirect:
            # Normal flow - set buyer details which will determine invoice type
            logger.info(f'before_insert: Normal flow - setting buyer details for {self.sales_invoice}')
            self._set_buyer_details(sales_invoice)
        else:
            # Clearance redirect - force simplified by not setting buyer VAT details
            logger.info(f'before_insert: Clearance redirect - creating simplified invoice for {self.sales_invoice}')
            self.buyer_vat_registration_number = None
            # Still set address details but skip VAT and additional IDs
    ...
    def after_insert(self):
        """Update ZATCA status in Sales Invoice after inserting additional fields."""
        self.update_zatca_status_in_invoice()

    def on_update(self):
        """Update ZATCA status in Sales Invoice when additional fields are updated."""
        # Check if integration_status changed
        if self.has_value_changed('integration_status'):
            self.update_zatca_status_in_invoice()

    def on_submit(self):
        """Update ZATCA status in Sales Invoice when additional fields are submitted."""
        self.update_zatca_status_in_invoice()
    ...
    def submit_to_zatca(self) -> Result[str, str]:
        ...
        logger.info(f'fatoora_server_url = {settings.fatoora_server_url}')
        ...
        logger.info(f'Invoice type {invoice_type} {integration_status}')

        if invoice_type == 'Standard' and integration_status == 'Clearance switched off':
            self._handle_clearance_switched_off(settings, token, secret)
    ...
    def _send_xml_via_api(
        ...
        # Update ZATCA status in the related Sales Invoice
        self.update_zatca_status_in_invoice()
        return integration_status

    def _handle_clearance_switched_off(self, settings: ZATCABusinessSettings, token: str, secret: str):
        """
        Handle the creation of a simplified invoice when a standard invoice receives 
        'Clearance switched off' status from ZATCA.
        """
        logger.info(f'Creating simplified invoice for clearance switched off standard invoice {self.sales_invoice}')
        
        # Use background job to avoid deadlock issues
        frappe.utils.background_jobs.enqueue(
            _create_simplified_invoice_for_clearance_switched_off,
            doc_data={
    ...
    def update_zatca_status_in_invoice(self):
        """Update the ZATCA status in the related Sales Invoice."""
        if not self.sales_invoice or not self.invoice_doctype:
            return
        ...
        try:
            frappe.db.set_value(
                ...
                self.sales_invoice,
                'custom_zatca_status',
                self.integration_status or '',
                update_modified=False
            )
        except Exception as e:
            frappe.log_error(
                message=f"Failed to update ZATCA status for {self.invoice_doctype} {self.sales_invoice}: {str(e)}",
...
def _create_simplified_invoice_for_clearance_switched_off(doc_data: dict):
    """
    Background job function to create a simplified invoice for clearance switched off standard invoices.
    This avoids deadlock issues by running in a separate transaction.
    """
    try:
        logger.info(f'Background job: Creating simplified invoice for clearance switched off standard invoice {doc_data["sales_invoice"]}')
        
        # Create new simplified invoice additional fields
        new_siaf = SalesInvoiceAdditionalFields.create_for_invoice(doc_data['sales_invoice'], doc_data['invoice_doctype'])
        
        # Set the dedicated clearance_redirect field
        new_siaf.clearance_redirect = True
        
        logger.info(f'Background job: Set clearance_redirect = {new_siaf.clearance_redirect} to force simplified invoice creation')
        
        # Insert the new invoice (this will trigger before_insert logic)
        new_siaf.insert()
        
        # Verify it was created as simplified
        settings = ZATCABusinessSettings.for_invoice(new_siaf.sales_invoice, new_siaf.invoice_doctype)
        invoice_type = new_siaf._get_invoice_type(settings) if settings else 'Unknown'
        logger.info(f'Background job: Created invoice as type: {invoice_type}, transaction type: {new_siaf.invoice_type_transaction}')
        
        # Use the proper background job pattern for submission (like fix_rejection does)
        if settings and settings.is_live_sync:
            frappe.utils.background_jobs.enqueue(_submit_additional_fields, doc=new_siaf, enqueue_after_commit=True)
            logger.info(f'Background job: Enqueued submission for simplified invoice {new_siaf.name}')
        else:
            logger.info(f'Background job: Created simplified invoice {new_siaf.name} but not submitting (live_sync disabled)')
        
    except Exception as e:
        logger.error(f'Background job: Failed to create simplified invoice for {doc_data["sales_invoice"]}: {str(e)}', exc_info=True)
